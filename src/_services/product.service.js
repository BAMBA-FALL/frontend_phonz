...
// Nouvelle méthode pour récupérer les produits par sous-catégorie
const getProductsBySubcategory = async (subcategoryId) => {
  try {
    const response = await Axios.get(`/api/products/subcategory/${subcategoryId}`);
    console.log('Response from getProductsBySubcategory:', response.data);
    return response.data;
  } catch (error) {
...
const getProductsByCategory = async (categoryId) => {
  try {
    ...
    console.log('Response from getProductsByCategory:', response.data);
    return response.data;
  } catch (error) {
